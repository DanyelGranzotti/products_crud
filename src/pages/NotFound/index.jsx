const NotFound = ({}) => {
  return (
    <div
      className="d-flex flex-column justify-content-center align-items-center w-100 text-center"
      style={{ height: "100vh" }}
    >
      <h1>Trabalho em progresso</h1>
      <p className="d-flex py-5 px-4 text-center w-50  justify-content-center">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla soluta
        nihil voluptate et excepturi modi dignissimos labore vero eligendi?
        Rerum esse autem, dolores earum quis blanditiis soluta molestias
        necessitatibus. Totam.
      </p>
    </div>
  );
};

export default NotFound;
